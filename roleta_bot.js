...
// Adicionar no início do código, junto com as outras variáveis de estado
let ultimaVitoria = null;
...
async function processarEstrategiaCores(res) {
  ...
  if (!alertaAtivo && !corAlvo && !colunaAlvo && !duziaAlvo && historico.length >= 3) {
    // CORREÇÃO: Verificar se algum dos números é o último vencedor
    if (ultimaVitoria && ultimaVitoria.numero === historico[0].numero) {
      console.log(`Ignorando verificação, pois o primeiro número (${historico[0].numero}) é o último vencedor.`);
      return;
    }
  ...
  // Primeira rodada após detectar padrão para cores (G0)
    ...
    if (res.numero === 0) {
      ...
      // CORREÇÃO: Marcar este número como vencedor em vez de limpar o histórico
      ultimaVitoria = {
        numero: res.numero,
        cor: res.cor
      };
  ...
  // Segunda rodada após detectar padrão para cores (G1)
    ...
    if (res.numero === 0) {
      ...
      // Após uma derrota, não marcamos vitória, apenas resetamos o alerta
      resetarAlertaCores();
    }
  }
}

// Função para resetar alerta de cores
...
async function processarEstrategiaColunas(res) {
    ...
    if (!alertaAtivo && !corAlvo && !colunaAlvo && !duziaAlvo && ultimosOitoNumeros.length === 8) {
      ...
      if (colunasDistintas.length === 2) {
        // MODIFICAÇÃO: Agora utilizamos as colunas que estão aparecendo como alvo
        alertaAtivo = true;
        colunaAlvo = colunasDistintas;  // Agora colunaAlvo é um array com as colunas presentes
...
async function processarEstrategiaDuzias(res) {
    ...
    if (!alertaAtivo && !corAlvo && !colunaAlvo && !duziaAlvo && ultimosOitoNumeros.length === 8) {
      ...
      if (duziasDistintas.length === 2) {
        // MODIFICAÇÃO: Agora utilizamos as dúzias que estão aparecendo como alvo
        alertaAtivo = true;
